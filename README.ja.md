...
2. **Stage 1 (体感1秒の主役)**: 10秒回答だけを超短文でストリーミング生成
3. **Stage 2 (後追い)**: 30秒回答 + 深掘りQ&A（3問）を追記
...
- **メニューバー**: 聞き取り開始/停止、オーバーレイ表示切替、テストモード、設定、終了
- **ダッシュボード（日本語UI）**:
  - 接続状態（プロキシ/文字起こし/生成）
  - 入力デバイス名
